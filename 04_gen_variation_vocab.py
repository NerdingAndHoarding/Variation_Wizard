...
class App:
    def __init__(self, root):
        ...
        tk.Button(root, text="GENERATE", command=self.generate).grid(row=2, column=0, columnspan=3)
    ...
    def generate(self):
        ...
        # =====================================================
        # waveData.txt (no WAV dependency anymore)
        # =====================================================
        with open(out_dir / "waveData.txt", "w", encoding="utf-8") as f:
            for i, (old, _) in enumerate(arrays, start=1):
                f.write(f"z_{i:03d} = ({old})\n")
